...
public class Model extends Observable {
    ...
    public boolean tilt(Side side) {
        ...
        for (int col = 0; col < _board.size(); col += 1) {
            for (int row = _board.size() - 1; row >= 0; row -= 1) {
                ...
                if (t != null) {
                    ...
                    if (nextPos >= row) { //check that the tile is below the current tile position
                        _board.move(col, nextPos, t);
                        changed = true;

                        nextPos -= 1;
                        if (nextPos < 0 || _board.tile(col, nextPos) != null) {
                            break;
                        }
                    }
                }
            ...
            for (int row = 3; row >= 1; row -= 1) {
                ...
                Tile nextTile = _board.tile(col, row - 1);
                if (nextTile != null && currTile.value() == nextTile.value()) {
                    // Merge and change score
                    _board.move(col, row, nextTile);
                    _score += currTile.value() * 2;

                    // Update positions of merged tiles
                    for (int newCol = row - 1; newCol >= 0; newCol -= 1) {
                        Tile newTile = _board.tile(col, newCol);
                        if (newTile != null) {
                            _board.move(col, newCol + 1, newTile);
                        }
                    }
                    changed = true;
                }
        ...
        _board.setViewingPerspective(Side.NORTH); //change back the perspective
        ...
        private int findNextEmptyPosition(int col, int row){
            int nextPos = 3;
